...
from code_utils.constants import (
    USER_TABLE, 
    LOGIN_URL,
    SIGNUP_URL
)
...
from decouple import config
from code_utils.stripe import get_create_checkout_session_url
import bcrypt
import pandas as pd
...
engine = pg.connect(dbname='postgres', user='postgres', host='localhost', port=5432, password='grapior0896')
psql_engine = create_engine(f"postgresql+psycopg2://{config('DB_USER')}:{config('DB_PWD')}@{config('DB_HOST')}:{config('DB_PORT')}/{config('DB_NAME')}")
#conn = psql_engine
stripe.api_key = config('SECRETE_API_KEY')
    ...
    def register_user(self, username, email, password):
        ...
        if user_exists:
            ...
            row = {
                ...
                'status': 1, # 0 = inactive, 1 = active, 2 = cancelled
